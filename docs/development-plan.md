...
- 首页阶段二：继续显示与阶段一完全相同的当前主题场景图和静态可读性蒙版；图片的位置、大小、裁切、亮度、饱和度、模糊、滤镜和缩放均不因滚动改变。标题与首屏终端先快速淡出，圆形普通头像从页面中下方升起并左移，随后冷色终端式个人名片展开并逐行打字；动画只作用于 HTML 内容层。滚轮输入采用阈值提交，未达到阈值时仅允许轻微探出并回弹，不将名片可见度连续绑定到每一小段滚动。
...
| 组件 | 职责/输入 | 客户端 | 响应式与冲突 |
|---|---|---|---|
| `HomeScrollScene` | 组织三阶段语义、滚轮意图阈值与回弹/提交状态；阶段一显示标题，阶段二按“标题退出→头像升起左移→终端名片展开打字”一次播放，阶段三让整个主视觉区域离场 | P5 核心 V1 | 手机缩短 sticky 区；reduced motion 改自然流；不得修改 `HomeHeroScene` 图片的 transform、position、filter、blur、brightness、saturation、scale 或裁切 |
...
| `ProfileCard` | 普通圆形作者头像与冷色终端式资料窗；头像升起、左移，资料窗再展开并逐行打字；不得再次使用从主视觉拆出的半身图 | P5 只做有限状态动画 | 桌面以整体居中构图并避开主体，手机改为头像在上、终端在下或进入正常流；不得通过移动背景来给卡片腾位 |
...
- 动画：快速反馈约 140–180ms，常规卡片/面板约 220–320ms；首页阶段二是约 2 秒的有限编排动画，由滚轮阈值一次提交而非逐像素 scrub，统一 `--ease-standard` 和 `--ease-emphasized`。
...
| 范围 | 主要布局 |
|---|---|
| 宽屏桌面（约 ≥1280） | 1200px 左右内容容器；当前主题完整场景图铺满主视觉。夜间图按已确认位置保留人物、光环、主要翅膀、手和花朵；日间图按自身主体设置焦点；阶段一保留标题安全区，阶段二的头像/终端组合从中央起势并落在不遮挡主体的中央至右侧区域；文章正文 + 右侧 sticky TOC；项目 3 列 |
...
|---|---|---|---|---|---|---|
| 夜间完整场景图 | 沿用已确认桌面构图：标题使用左侧可读区；阶段二头像/终端组合使用中央至右侧安全区，不遮住脸部、光环中心和花朵 | `object-fit: cover`；1440×900 已实测，左右背景略裁；为常用桌面断点固定 `object-position` | 优先复用同图，焦点为脸、上半身、光环中心和花朵；允许裁掉部分翅膀和城市背景 | 当前为 10000×5012 JPG、sRGB、无 Alpha；Astro 输出响应式 AVIF/WebP | 夜间专用；使用深色 UI 与静态蒙版 | 默认 1 张 |
| 日间完整场景图 | 优先选择明亮冷色、非纯白曝亮的完整插画；需要阶段一标题安全区，以及阶段二头像/终端组合的中央至右侧活动区，关键主体不得被 UI 遮挡。无需与夜间人物相同，但整体视觉重量应接近 | `object-fit: cover`；在 1440×900、1024×768 检查主体、标题和终端名片；每主题可有独立 `object-position` | 优先复用同图，在 390×844、360×800 检查主体与 UI；禁止滚动时重算裁切 | JPG/PNG、sRGB；Astro 输出响应式 AVIF/WebP | 日间专用；使用浅色 UI 与静态蒙版 | 默认 1 张 |
...
- **工作：** sticky + 单 rAF 管理几何；滚轮方向与累计量进入有限状态机。未达到阈值时头像只轻微探出并回弹，达到阈值后一次提交“标题/首屏终端快速退出→圆形头像从中下方升起并左移→冷色终端名片展开→文本逐行打字”，不把组件透明度逐像素绑定到滚动。阶段三才让整个主视觉区域离场。禁止修改 `HomeHeroScene` 图片的 transform、position、filter、blur、brightness、saturation、scale 或裁切；手机缩短，低高度/低性能设备减少或取消 sticky 长滚动，reduced motion/无 JS 使用自然流。
- **验证：** 三阶段顺序、阈值前不提交、输入中断回弹、达到阈值只提交一次、反向输入返回阶段一；light/dark 分别断言阶段切换前后引用同一个图片 DOM 实例、`currentSrc` 不变、bounding box 不变、`object-position` 不变，computed `transform`、`filter`、`scale`、`brightness`、`saturation`、`blur` 均不变，且当前主题大图没有新增网络请求；只允许标题、首屏终端、头像、终端名片和阶段三主视觉容器的前景状态变化。在阶段一和阶段二分别切换主题，确认只因用户主题动作更换资源且保持当前阶段。截图对比只作合理容差的辅助；另覆盖键盘提交、低端设备、低高度、resize、JS 禁用、reduced motion、无横向溢出。
- **前置：** P4。
