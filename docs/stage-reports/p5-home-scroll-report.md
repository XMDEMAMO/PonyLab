...
用户在 P6 合并后对阶段二的实际体验提出了定向反馈，P7 分支先完成以下必要修正，再进入文章阅读阶段：

- 原本随滚动进度逐段显隐的名片改为有阈值的有限状态动画；滚轮累计不足时头像只轻微探出，停止后回弹，不会停留在半透明中间态。
- 达到阈值才一次提交完整时序：标题与首屏终端快速退出，圆形作者头像从页面中下方升起到中央、再向左停靠，随后冷色终端资料窗展开并逐行打字。
- 反向滚轮达到阈值后统一收回；Arrow/Page/Space 提供键盘等价操作。reduced motion 和自然流降级仍直接展示完整信息。
- `HomeHeroScene` 仍完全独立，场景图的 DOM、资源、bounding box、`object-position`、transform 与 filter 不随动画变化。
- 新增 `tests/unit/p5-motion-contract.test.ts`，扩展滚轮状态机 unit 和 Playwright 阈值/回弹/提交测试；light/dark、桌面/手机、无 JavaScript 与 reduced motion 回归均保留。

该修正属于用户确认后的 P5 体验完善，没有改变三阶段语义、双场景图方案或阶段边界；最终代码随 `codex/p7-article-reading` 分支交付。
...
- `src/utils/home-scroll.ts` 将 sticky 容器几何映射为 `0—1` 进度，集中计算第三阶段离场，同时用纯函数累计滚轮方向与阈值。
- 阶段一保留标题与终端；阶段二达到阈值后播放“标题退出→头像升起左移→终端资料窗展开打字”的固定时序；阶段三让主视觉容器开始离场，并在 sticky 区域释放后自然上移，与文章过渡区直接衔接。
- scroll/resize 只通过一个 `requestAnimationFrame` 合并几何更新；仅增强模式下的 wheel 意图监听需要阻止阈值内默认滚动，以实现回弹和一次提交。没有引入第三方动画依赖。
...
- 390×844 下，头像与终端资料窗改为上下堆叠并缩短位移，整体避开两张构图中的关键人物面部；这只调整 HTML 前景，不移动背景。
...
- 为同时保护两张移动构图的主体面部，阶段二在窄屏采用头像在上、终端资料窗在下的紧凑布局。图片焦点、裁切和视觉状态均未改变。
